...
time_steps = int(np.ceil(simulation_time / time_step_length))

t_i = np.linspace(time_of_launch, time_of_launch + simulation_time, time_steps)
inter_func = interpolate.interp1d(t_all, r_all, axis=-1)
r_interpolated = inter_func(t_i)

r_p_vec = r_interpolated[:,0,0]

theta = 5*np.pi/4
radius_vec = home_planet_R * np.array([np.cos(theta), np.sin(theta)])
...
craft_position = spacecraft_position(dist, r_interpolated[:,:,0])
# mission.verify_manual_orientation(craft_position, craft_velocity, 0)

if __name__ == '__main__':
    t, v_craft, r_craft, r_i = trajectory(time_of_launch, craft_position, craft_velocity, simulation_time, time_step_length)

    plt.figure()
    plt.title(f'Home, neighbor and destination planet position at time $t={min_dist_time:.3f}\,yrs$')
    plt.plot([0,0], [0,0], 'ko', label='Star')
    plt.plot(r_planets[0,0,:], r_planets[1,0,:], 'k')
    plt.plot(r_planets[0,1,:], r_planets[1,1,:], 'k')
    plt.plot(r_planets[0,2,:], r_planets[1,2,:], 'k')
    plt.plot(r_planets[0,0,min_dist_index], r_planets[1,0,min_dist_index], 'ro', label='Home')
    plt.plot(r_planets[0,1,min_dist_index], r_planets[1,1,min_dist_index], 'bo', label='Neighbor')
    plt.plot(r_planets[0,2,min_dist_index], r_planets[1,2,min_dist_index], 'go', label='Destination')
    plt.xlabel('AU')
    plt.ylabel('AU')
    plt.legend(loc='lower left')
    # plt.savefig('min-dist-plot.png')
    plt.show()

    # plt.figure()
    # plt.plot(r_i[0,0,:], r_i[1,0,:])
    # plt.plot(r_i[0,1,:], r_i[1,1,:])
    # plt.plot(r_i[0,6,:], r_i[1,6,:])
    #
    # plt.plot(r_i[0,0,0], r_i[1,0,0], 'ro')
    # plt.plot(r_i[0,1,0], r_i[1,1,0], 'bo')
    # plt.plot(r_i[0,6,0], r_i[1,6,0], 'go')
    # plt.plot(r_i[0,0,-1], r_i[1,0,-1], 'ro')
    # plt.plot(r_i[0,1,-1], r_i[1,1,-1], 'bo')
    # plt.plot(r_i[0,6,-1], r_i[1,6,-1], 'go')
    # plt.plot([0,0], [0,0], 'ko')
    #
